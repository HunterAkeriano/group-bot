...
async function generateStoryParts(ctx, text) {
    ...
    const storyPrompt = `
Створи дуже захоплюючу, клікбейтну історію українською мовою на тему: "${text}". Розкажи її від першої особи (про себе, використовуючи "Я").
Історія має бути розділена на 3 частини. Кожна частина має бути близько 1000 символів.
1. ПОЧАТОК: Захоплююче введення, що створює інтригу. Закінчується на піку події або великому питанні.
2. СЕРЕДИНА: Розвиток сюжету, який лише посилює проблему або додає нові шокуючі деталі. Закінчується на КРАЙНЬОМУ кліфгенгері, змушуючи чекати.
3. РОЗВ'ЯЗКА: Несподівана, але логічна розв'язка та висновок (мораль).
